...
def model(X, w_h, w_h2, w_o, p_keep_input, p_keep_hidden):
    # Add alyer name scopes for better graph visualization
...
with tf.name_scope("cost"):
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logist(py_x, Y))
    train_op = tf.train.RMSPropOptimizer(0.001, 0.9).minimize(cost)
    # Add scaler summary for cost tensor
...
with tf.name_scope("accuracy"):
    ...
    tf.scalar_summary("accuracy", acc_op)                        # Add scalar summary for accuracy tensor
...
with tf.Session() as sess:
    ...
    # Step 11 - You need to initalize all variablese
    ...
    for i in range(100):
        ...
        writer.add_summary(summar, i) # Writer summar
